...
namespace Security.API.EndPoints
{
    public static class RoleEndPoints
    {
        public static void MapRoleEndPoints(this IEndpointRouteBuilder route)
        {
            ...
            route.MapPost("/role", async (CreateRoleRequest request, IMediator mediator, CancellationToken cancellationToken) =>
            {
            ...
            route.MapPut("/role/{id}", async (string id, UpdateRoleRequest request, IMediator mediator, CancellationToken cancellationToken) =>
            {
                request.Id = id;
                var result = await mediator.Send(request, cancellationToken);
                return result.Succeeded ? Results.NoContent() : Results.BadRequest(result.Message);
